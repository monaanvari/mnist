...
def softmax(x):
    ...
    return e_x / e_x.sum(axis=1, keepdims=True)[:, None]
...
def backward_prop(data, labels, params, forward_prop_func):
    ...
    """
    # *** START CODE HERE ***
    a, output, loss = forward_prop_func(data, labels, params)
    w1 = data.T.dot(
        (output - labels).dot(params['W2'].T) * a * (1 - a)) / data.shape[0]
    b1 = np.sum(
        (output - labels).dot(params['W2'].T) * a * (1 - a), axis=0) / data.shape[0]
    w2 = a.T.dot(output - labels) / data.shape[0]
    b2 = np.sum(output - labels, axis=0) / data.shape[0]
...
def gradient_descent_epoch(train_data, train_labels, learning_rate, batch_size, params, forward_prop_func, backward_prop_func):
    ...
    for i in range(train_data.shape[0] // batch_size):
        ...
        labels = train_labels[i * batch_size: i * batch_size + batch_size]
        grads = backward_prop_func(batch, labels, params, forward_prop_func)
...
def main(plot=True):
    ...
    baseline_acc = run_train_test(
        'baseline', all_data, all_labels, backward_prop, args.num_epochs, plot)
